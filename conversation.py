...
from adapter.sdwebui.api import SDwebUIAdapter
...
class ConversationContext:
    ...
    def __init__(self, _type: str, session_id: str):
        ...
        elif _type == 'sdwebui-api':
            self.adapter = SDwebUIAdapter(self.session_id)
        else:
        ...
        try:
            ...
            self.sdwebui_api = SDwebUIAdapter(session_id)
    ...
    async def ask(self, prompt: str, chain: MessageChain = None, name: str = None):
        ...
        for prefix in config.trigger.prefix_image:
            if prompt.startswith(prefix) and not isinstance(self.adapter, YiyanAdapter):
                if not self.openai_api and not self.sdwebui_api:
                    yield "没有开启画图功能！"
                prompt_0 = prompt.removeprefix(prefix)
                # if message.photo:
                #     # 如果是图片消息
                #     photo_id = message.photo[-1].file_id
                #     photo_path = bot.get_file(photo_id).download()
                #     text = message.caption or ''
                #     # 调用图片处理函数
                print('MessageChain:', chain, 'user:', name)
                logger.debug('MessageChain:', chain, 'user:', name)
                if chain.has(GraiaImage):
                    image = chain.get_first(GraiaImage)
                    logger.debug("chain.get_first(GraiaImage)")
                    raw_bytes = io.BytesIO(await image.get_bytes())
                    raw_image = Image.open(raw_bytes)
                    image_data_path = await self.sdwebui_api.image_variation(src_img=raw_image, prompt=prompt_0)
                else:
                    image_data_path = await self.sdwebui_api.image_creation(prompt=prompt_0)
