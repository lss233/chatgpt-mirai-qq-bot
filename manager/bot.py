...
from config import OpenAIAuthBase, OpenAIAPIKey, Config, BingCookiePath, BardCookiePath, YiyanCookiePath, ChatGLMAPI, \
    PoeCookieAuth, SDwebUIAPI, SDwebUIAuths
...
class BotManager:
    ...
    sdwebui: List[SDwebUIAPI]
    """sdwebui Account Infos"""
    ...
    def __init__(self, config: Config) -> None:
        ...
        self.sdwebui = config.sdwebui.accounts if config.sdwebui else []
        try:
            os.mkdir('data')
            logger.warning(
    ...
    async def login(self):
        ...
        if not self.config.response.default_ai:
            ...
            elif len(self.bots['sdwebui-api']) > 0:
                self.config.response.default_ai = 'sdwebui-api'
    ...
    def login_sdwebui(self):
        for i, account in enumerate(self.sdwebui):
            logger.info("正在解析第 {i} 个 SDwebUI 账号", i=i + 1)
            try:
                self.bots["sdwebui-api"].append(account)
                logger.success("解析成功！", i=i + 1)
            except Exception as e:
                logger.error("解析失败：")
                logger.exception(e)
        if len(self.bots) < 1:
            logger.error("所有 SDwebUI 账号均解析失败！")
        logger.success(f"成功解析 {len(self.bots['sdwebui-api'])}/{len(self.sdwebui)} 个 SDwebUI 账号！")
