...
from framework.exceptions import PresetNotFoundException, BotRatelimitException, ConcurrentMessageException, \
...
async def handle_message(_respond: Callable, session_id: str, prompt: str,
    ...
    # 回滚会话
        ...
        await _respond(constants.config.response.rollback_success if await conversation_context.rollback() else constants.config.response.rollback_fail.format(
    ...
    # 切换语音
        ...
        conversation_context.conversation_voice = await conversation_context.tts_engine.choose_voice(new_voice)
        await _respond(f"已切换至 {conversation_context.conversation_voice.full_name} 语音，让我们继续聊天吧！")
        return

    # 加载预设
    elif preset_search := re.search(constants.config.presets.command, prompt):
    ...
    async def respond(msg: Optional[MessageChain]):
        ...
        if conversation_context.conversation_voice and conversation_context.conversation_voice:
            tasks = []
            for elem in msg:
                task = asyncio.create_task(conversation_context.tts_engine.speak(elem, conversation_context.conversation_voice))
                tasks.append(task)
            while tasks:
