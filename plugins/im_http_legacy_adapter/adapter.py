...
class HttpLegacyAdapter(IMAdapter):
    ...
    def setup_routes(self):
        ...
        async def v2_chat():
            ...
            bot_request = V2Request(session_id, message.sender.username, data.get('message', ''), request_time)
        ...
        async def v2_chat_response():
            ...
            if bot_request is None:
                return ResponseResult(message="没有更多了！", result_status="FAILED").to_json()
    ...
    async def cleanup_expired_requests(self):
        ...
        while True:
            now = time.time()
            expired_keys = [
                key for key, req in self.request_dic.items()
