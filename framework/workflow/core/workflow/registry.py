...
class WorkflowRegistry:
    ...
    def get_workflow_path(cls, group_id: str, workflow_id: str) -> str:
        ...
        if os.path.commonprefix((os.path.realpath(final_path), cls.WORKFLOWS_DIR)) != cls.WORKFLOWS_DIR:
        ...
        if not re.match(r'^[a-zA-Z0-9_-]+$', workflow_id):
            ...
            raise ValueError(f"Invalid symbols in workflow path: {''.join(invalid_chars)}")
    ...
    def register(self, group_id: str, workflow_id: str, workflow_builder: WorkflowBuilder):
        """注册一个工作流"""
        full_name = f"{group_id}:{workflow_id}"
        if full_name in self._workflows:
    ...
    def register_preset_workflow(self, group_id: str, workflow_id: str, workflow_builder: WorkflowBuilder):
        """预设工作流注册，当用户保存了同 id 的工作流时，则会不注册"""
        full_name = f"{group_id}:{workflow_id}"
        if full_name in self._workflows:
            self.logger.debug(f"Preset workflow {full_name} already registered, skipping")
            return
        self._workflows[full_name] = workflow_builder
        self.logger.info(f"Registered preset workflow: {full_name}")
        
    def get(self, name: str, container: DependencyContainer = None) -> Optional[WorkflowBuilder]:
        """获取工作流构建器或实例"""
