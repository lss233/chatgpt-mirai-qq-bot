...
def test_input_validation():
    ...
    input_obj = Input(name="input1", label="输入1", data_type=int, description="An integer input")
    ...
    nullable_input = Input(name="input2", label="输入2", data_type=int, description="A nullable integer input", nullable=True)
    assert nullable_input.validate(None) is True

def test_output_validation():
    """Test output validation."""
    output_obj = Output(name="output1", label="输出1", data_type=str, description="A string output")
